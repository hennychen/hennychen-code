...
class JSON_API_Core_Controller {
  ...
  public function get_favorites(){
      ...
      if (!is_plugin_active('wp-favorite-posts/wp-favorite-posts.php')) {
          $json_api->error("Not found.wp-favorite-posts");
          return;
      }
...
//      $post = $json_api->introspector->get_current_post_byid($id);
//      if ($post) {
//
//          $response = array(
...
//  }
  ...
  public function get_author_index() {
    ...
    //获取用户数量
  public  function  get_author_counts(){
     global $json_api;
      $authorcount = $json_api->introspector->get_author_count();
      return array(
          'count' =>$authorcount
      );
  }
//获取指定用户的评论通过用户名称 或者 用户email 或者用户id
  public function get_author_comments() {
        global $json_api;
        extract($json_api->query->get(array('author_id', 'name','email')));

        if($author_id||$name||$email){
            if($author_id){
                 $comments = $json_api->introspector->get_author_comments_by_id($author_id);
                 return array(
            ...
            elseif($name){
                $comments = $json_api->introspector->get_author_comments_by_name($name);
                return array(
                    'count' => count($comments),
                    'comments' => $comments
                );
            ...
            elseif($email){
                $comments = $json_api->introspector->get_author_comments_by_email($email);
                return array(
                    'count' => count($comments),
                    'comments' => $comments
                );
            ...
            $json_api->error("Include 'author_id' or 'name' or 'email' var in your request.");
        }
  }
//获取用户的所有回复
  public function get_author_replay(){
      global $json_api;
      $authorId = $json_api->query->id;
      $comments = $json_api->introspector->get_replay_by_id($authorId);
  ...
  public function register_user(){
        global $json_api;
        extract($json_api->query->get(array('name', 'email','pwd')));
        if($name && $email && $pwd){
            $result = $json_api->introspector->registerUser($name,$pwd,$email);
            return array(
                'result' => $result
            );
        }
        else{
            $json_api->error("include name,email,pwd var in your request");
        }
    }
