...
$wp_category->term_icon = get_option('z_taxonomy_image'.$wp_category->term_id);
...
//   * 根据id获取文章信息
//   * */
//  public function get_current_post_byid($id) {
//    global $json_api;
//    extract($json_api->query->get($id));
//    if ($id ) {
//        $posts = $this->get_posts(array(
//            'p' => $id
//        ), true);
...
//}
  ...
  //获取用户收到的评论回复
  public function get_replay_by_id($user_id){
      global $wpdb;
      $wp_comments = $wpdb->get_results($wpdb->prepare("SELECT * from  wordpressdb.wp_comments WHERE comment_parent IN
        (SELECT distinct comment_ID FROM wordpressdb.wp_comments WHERE user_id=%d)", $user_id));
      $replay_comments = array();
      foreach($wp_comments as $wp_comment){
          $replay_comments[] = new JSON_API_Comment($wp_comment);
      }
      return $replay_comments;
  }

  //通过用户id获取评论信息
  public function get_author_comments_by_id($user_id) {
     global $wpdb;
     $wp_comments = $wpdb->get_results($wpdb->prepare("
      SELECT *
      FROM $wpdb->comments
    ...
    //通过用户name获取评论信息
  public function get_author_comments_by_name($user_name) {
    global $wpdb;
    $wp_comments = $wpdb->get_results($wpdb->prepare("
      SELECT *
      FROM $wpdb->comments
    ...
    public function get_author_comments_by_email($email) {
        global $wpdb;
        $wp_comments = $wpdb->get_results($wpdb->prepare("
      SELECT *
      FROM $wpdb->comments
    ...
    public function registerUser($name,$pwd,$email){
        global $wpdb;
